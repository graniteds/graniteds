...
deeply into the JPA provider to build the change set objects. The server-to-client change sets are currently implemented only for Hibernate 3 and Hibernate 4.
...
Obviously that means that the server method that is called knows what to do with the +ChangeSet+ objects. Tide provides the
+ChangeSetApplier+ API that can be used on the server :
