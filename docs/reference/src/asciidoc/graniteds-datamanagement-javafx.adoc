...
* ++publish++: the publishing mode. +PublishMode.MANUAL+ means that you will have to trigger the dispatch manually, for example in an interceptor. 
    +$$PublishMode.ON_SUCCESS$$+ means that Tide will automatically dispatch the updates on any successful call. 
    +$$PublishMode.ON_COMMIT$$+ is not yet implemented. 
* ++params++: a filter class that will define to which updates are sent to which clients. By default there is no filtering, otherwise see below 
